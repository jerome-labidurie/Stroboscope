...
const byte PIN_KNOB_B = 3;          //  .. B switch
...
void display (void) {
	char line[17];
	sprintf (line, "Freq: %4d.%02d Hz", (int)FlashFreq,(int)(100.0 * (FlashFreq - trunc(FlashFreq))));
// 	lcd.clear();
	lcd.setCursor(0,0);
	lcd.print(line);
	sprintf (line, "Incr: %4d.%02d Hz", (int)FreqIncr,(int)(100.0 * (FreqIncr - trunc(FreqIncr))));
	lcd.setCursor(0,1);
	lcd.print(line);
}
        ...
        if (PrevButton != Button) {
            if (Button == N_BUTTONS) {
              printf("Button %d released\n",PrevButton);
				  if (FreqIncr == 0.01) {
					  FreqIncr = 0.1;
				  } else if (FreqIncr == 0.1) {
					  FreqIncr = 1.0;
				  } else if (FreqIncr == 1.0) {
					  FreqIncr = 10.0;
				  } else if (FreqIncr == 10.0) {
					  FreqIncr = 100.0;
				  } else {
					  FreqIncr = 0.01;
				  }
...
//                     FreqIncr = 0.01;
        ...
        if (KnobCounter) {
            ...
            printf("Frequency: %d.%02d, Inc:%d.%02d\n",
                   (int)FlashFreq,(int)(100.0 * (FlashFreq - trunc(FlashFreq))),
						 (int)FreqIncr,(int)(100.0 * (FreqIncr - trunc(FreqIncr)))
						);
				display ();
        }
